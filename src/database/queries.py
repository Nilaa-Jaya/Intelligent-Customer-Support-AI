...
class UserQueries:
    ...
    @staticmethod
    def create_user(db: Session, user_id: str, name: str = None, email: str = None, is_vip: bool = False) -> User:
        """Create a new user"""
        user = User(
            user_id=user_id,
            name=name,
...
class FeedbackQueries:
    ...
    def get_average_rating(db: Session, days: int = 7) -> Optional[float]:
        ...
        result = db.query(func.avg(Feedback.rating)).filter(
...
class AnalyticsQueries:
    ...
    def update_hourly_analytics(db: Session):
        ...
        conversations = db.query(Conversation).filter(
            Conversation.created_at >= current_hour_start
        ).all()
        
        # Calculate metrics
        analytics.total_queries = len(conversations)
        analytics.technical_queries = sum(1 for c in conversations if c.category == "Technical")
        analytics.billing_queries = sum(1 for c in conversations if c.category == "Billing")
        analytics.general_queries = sum(1 for c in conversations if c.category == "General")
        analytics.account_queries = sum(1 for c in conversations if c.category == "Account")
        
        analytics.positive_count = sum(1 for c in conversations if c.sentiment == "Positive")
        analytics.neutral_count = sum(1 for c in conversations if c.sentiment == "Neutral")
        analytics.negative_count = sum(1 for c in conversations if c.sentiment == "Negative")
        ...
        analytics.resolution_count = sum(1 for c in conversations if c.status == "Resolved")
        
        # Get feedback for this period
        feedbacks = db.query(Feedback).join(Conversation).filter(
            Conversation.created_at >= current_hour_start
        ).all()
    ...
    def get_analytics_summary(db: Session, days: int = 7) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_response_time": sum(c.response_time for c in conversations if c.response_time) / len(conversations) if conversations else 0,
            "escalation_rate": sum(1 for c in conversations if c.escalated) / len(conversations) * 100 if conversations else 0,
            "resolution_rate": sum(1 for c in conversations if c.status == "Resolved") / len(conversations) * 100 if conversations else 0,
